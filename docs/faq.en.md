...
By default there is no checking: this enterprise build checks for stable releases in the background only when the deployment presets a self-hosted update origin, and it never installs silently. A newer version requires confirmation. Before downloading, a native save dialog lets you choose the installer's directory and filename; cancelling it does not start a download. macOS downloads and opens a DMG; Windows downloads and starts an NSIS installer. After the upgrade and next launch, the app asks whether to delete or keep the installer. Network and download failures leave the current installation intact.

## Where can I download the app or report a problem?

This build is an enterprise self-hosted form: installers are distributed internally by the deploying organization, update checks are off by default, and only a self-hosted update origin can be preset (see `docs/enterprise-self-hosting.md`). Check the [troubleshooting section](user-guide.en.md#troubleshooting) first. If the problem remains, open a [GitHub Issue](https://github.com/anywhere-labs/deepseek-harness-desktop/issues/new/choose) with the operating system, app version, reproduction steps, and error details.
