...
const COPY: Record<DesktopLocale, EnterpriseLoginCopy> = {
  en: {
    ...
    disclosure: 'If your organization turns on session reporting, this client sends the organization server minimal session metadata — session start and end times and a plugin inventory hash. It never sends conversation titles, prompts, or token usage; and while reporting is on, an active session keeps model access alive. Reporting is off unless your administrator enables it.',
  ...
  zh: {
    ...
    disclosure: '若组织开启会话上报，本客户端仅向组织服务器发送最小会话元数据——会话起止时间与插件清单哈希；不会发送对话标题、提示词或用量数字。上报开启期间，保持会话活跃亦是模型访问的前提；管理员未开启时不发送任何上报。',
