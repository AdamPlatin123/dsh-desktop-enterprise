...
除非部署方在打包时预置了组织自建更新源（`DSH_ENTERPRISE_UPDATE_URL`），更新检查保持关闭；预置后，打包的 macOS 与 Windows 应用才会在启动 60 秒后查询 `{源点}/api/desktop/version`，并在每次检查完成六小时后再次查询。本构建的更新流程不会访问任何公共更新端点。每次 no-cache 请求的期限为 15 秒，会在 `X-DSH-Desktop-Version` 中携带当前安装版本，并与托盘中的 **Check for Updates…** 命令共用一个 in-flight operation。响应只有在包含规范的 stable Semantic Versioning 时才会被接受。后台检查遇到网络、HTTP、超时、无效响应、相同版本或服务端旧版本时保持静默。发现严格更新的后台版本时，应用会更新托盘并且每个版本只发送一次非阻塞原生通知，不会在其他应用上方弹出下载确认；点击通知会显示 DSH Desktop。手工检查一定会显示原生结果对话框：相同或旧版本会显示当前安装版本，失败会提示用户重试，严格更新的版本则显示 **Download** 或 **Later**。开发运行、未打包启动与 Linux 不会下载安装包。
...
（上游公共发布流程，不适用于本企业构建——本构建的更新源为自建。）Release operator 必须先发布两个平台产物，再让版本可被发现。产物与 download redirect 准备完成后，在 Upstash Redis console 中把 `deepseek-harness-desktop:release:version` 设置为规范的 stable 版本，例如 `SET deepseek-harness-desktop:release:version 2.0.1`。版本 API 会立即生效；key 缺失、服务不可用或值无效时，Desktop 不会显示任何提示。
